...
def ig_account_info(account_name, composio_account, prev, prev_key):
    """Profile stats from the JIT cache (API-slim policy 2026-09-11).

    INSTAGRAM_GET_USER_INFO is NO LONGER called on the dashboard refresh
    schedule. The single sanctioned caller is ig_post_slot.sh's JIT hook
    (ig_profile_stats.py) which runs immediately before each publishing
    window; freshness is governed by max_age_hours in ig_profile_stats.

    Args:
        prev: the previous data.json payload (for last-good fallback)
        prev_key: key of this account inside prev["ig"]["accounts"]
    """
    try:
        sys.path.insert(0, os.path.join(os.path.dirname(BASE), "IG"))
        from ig_profile_stats import load_profile  # noqa: E402
        stats = load_profile(account_name)
        if stats:
            return {
        ...
        print("ig_account_info cache read error:", e, file=sys.stderr)
    prev_acc = ((prev or {}).get("ig") or {}).get("accounts", {}).get(prev_key, {}).get("account")
    if prev_acc:
        return prev_acc  # last good data; caller marks entry stale
    return None
...
def main():
    ...
    for name, cfg in accounts.items():
        ...
        acc = ig_account_info(name, cfg.get("composio_account", ""), prev, name)
        entry = {
